...
#include "tinkerforge/brick_silent_stepper.h"
...
static void silent_stepper_callback_wrapper_under_voltage(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + SILENT_STEPPER_CALLBACK_UNDER_VOLTAGE];
...
static void silent_stepper_callback_wrapper_position_reached(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + SILENT_STEPPER_CALLBACK_POSITION_REACHED];
...
static void silent_stepper_callback_wrapper_all_data(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + SILENT_STEPPER_CALLBACK_ALL_DATA];
...
static void silent_stepper_callback_wrapper_new_state(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + SILENT_STEPPER_CALLBACK_NEW_STATE];
