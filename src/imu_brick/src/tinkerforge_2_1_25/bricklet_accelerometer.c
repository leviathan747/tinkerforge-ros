...
#include "tinkerforge/bricklet_accelerometer.h"
...
static void accelerometer_callback_wrapper_acceleration(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + ACCELEROMETER_CALLBACK_ACCELERATION];
...
static void accelerometer_callback_wrapper_acceleration_reached(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + ACCELEROMETER_CALLBACK_ACCELERATION_REACHED];
...
void accelerometer_register_callback(Accelerometer *accelerometer, int16_t callback_id, void *function, void *user_data) {
