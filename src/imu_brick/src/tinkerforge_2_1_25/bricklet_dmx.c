...
#include "tinkerforge/bricklet_dmx.h"
...
static void dmx_callback_wrapper_frame(DevicePrivate *device_p, uint16_t frame_length, uint16_t frame_chunk_offset, uint8_t frame_chunk_data[56], uint32_t frame_number) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + DMX_CALLBACK_FRAME];
...
static void dmx_callback_wrapper_frame_started(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + DMX_CALLBACK_FRAME_STARTED];
...
static void dmx_callback_wrapper_frame_available(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + DMX_CALLBACK_FRAME_AVAILABLE];
...
static void dmx_callback_wrapper_frame_low_level(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + DMX_CALLBACK_FRAME_LOW_LEVEL];
...
static void dmx_callback_wrapper_frame_error_count(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + DMX_CALLBACK_FRAME_ERROR_COUNT];
