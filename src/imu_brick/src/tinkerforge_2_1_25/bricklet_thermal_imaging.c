...
#include "tinkerforge/bricklet_thermal_imaging.h"
...
static void thermal_imaging_callback_wrapper_high_contrast_image(DevicePrivate *device_p, uint16_t image_chunk_offset, uint8_t image_chunk_data[62]) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + THERMAL_IMAGING_CALLBACK_HIGH_CONTRAST_IMAGE];
...
static void thermal_imaging_callback_wrapper_temperature_image(DevicePrivate *device_p, uint16_t image_chunk_offset, uint16_t image_chunk_data[31]) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + THERMAL_IMAGING_CALLBACK_TEMPERATURE_IMAGE];
...
static void thermal_imaging_callback_wrapper_high_contrast_image_low_level(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + THERMAL_IMAGING_CALLBACK_HIGH_CONTRAST_IMAGE_LOW_LEVEL];
...
static void thermal_imaging_callback_wrapper_temperature_image_low_level(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + THERMAL_IMAGING_CALLBACK_TEMPERATURE_IMAGE_LOW_LEVEL];

	callback->image_chunk_offset = leconvert_uint16_from(callback->image_chunk_offset);
	for (i = 0; i < 31; i++) callback->image_chunk_data[i] = leconvert_uint16_from(callback->image_chunk_data[i]);

	thermal_imaging_callback_wrapper_temperature_image(device_p, callback->image_chunk_offset, callback->image_chunk_data);
