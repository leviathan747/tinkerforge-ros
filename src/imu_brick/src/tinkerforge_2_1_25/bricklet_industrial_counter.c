...
#include "tinkerforge/bricklet_industrial_counter.h"
...
static void industrial_counter_callback_wrapper_all_counter(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_COUNTER_CALLBACK_ALL_COUNTER];

	if (callback_function == NULL) {
		return;
	}

	for (i = 0; i < 4; i++) callback->counter[i] = leconvert_int64_from(callback->counter[i]);

	callback_function(callback->counter, user_data);
}
...
static void industrial_counter_callback_wrapper_all_signal_data(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_COUNTER_CALLBACK_ALL_SIGNAL_DATA];

	if (callback_function == NULL) {
		return;
	}

	for (i = 0; i < 4; i++) callback->duty_cycle[i] = leconvert_uint16_from(callback->duty_cycle[i]);
	for (i = 0; i < 4; i++) callback->period[i] = leconvert_uint64_from(callback->period[i]);
	for (i = 0; i < 4; i++) callback->frequency[i] = leconvert_uint32_from(callback->frequency[i]);
	for (i = 0; i < 4; i++) unpacked_value[i] = (callback->value[i / 8] & (1 << (i % 8))) != 0;

	callback_function(callback->duty_cycle, callback->period, callback->frequency, unpacked_value, user_data);
}
...
void industrial_counter_register_callback(IndustrialCounter *industrial_counter, int16_t callback_id, void *function, void *user_data) {
