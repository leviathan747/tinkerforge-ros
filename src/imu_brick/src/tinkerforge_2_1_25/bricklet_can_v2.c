...
#include "tinkerforge/bricklet_can_v2.h"
...
static void can_v2_callback_wrapper_frame_read(DevicePrivate *device_p, uint8_t frame_type, uint32_t identifier, uint8_t data_length, uint8_t data_data[15]) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + CAN_V2_CALLBACK_FRAME_READ];
...
static void can_v2_callback_wrapper_frame_read_low_level(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + CAN_V2_CALLBACK_FRAME_READ_LOW_LEVEL];
