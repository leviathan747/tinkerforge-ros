...
#include "tinkerforge/bricklet_rs232.h"
...
static void rs232_callback_wrapper_read(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + RS232_CALLBACK_READ];
...
static void rs232_callback_wrapper_error(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + RS232_CALLBACK_ERROR];
...
void rs232_register_callback(RS232 *rs232, int16_t callback_id, void *function, void *user_data) {
