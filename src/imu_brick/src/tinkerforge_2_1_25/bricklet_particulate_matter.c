...
#include "tinkerforge/bricklet_particulate_matter.h"
...
static void particulate_matter_callback_wrapper_pm_concentration(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + PARTICULATE_MATTER_CALLBACK_PM_CONCENTRATION];
...
static void particulate_matter_callback_wrapper_pm_count(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + PARTICULATE_MATTER_CALLBACK_PM_COUNT];
