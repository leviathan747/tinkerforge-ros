...
#include "tinkerforge/bricklet_accelerometer_v2.h"
...
static void accelerometer_v2_callback_wrapper_acceleration(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + ACCELEROMETER_V2_CALLBACK_ACCELERATION];
...
static void accelerometer_v2_callback_wrapper_continuous_acceleration_16_bit(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + ACCELEROMETER_V2_CALLBACK_CONTINUOUS_ACCELERATION_16_BIT];

	if (callback_function == NULL) {
		return;
	}

	for (i = 0; i < 30; i++) callback->acceleration[i] = leconvert_int16_from(callback->acceleration[i]);
...
static void accelerometer_v2_callback_wrapper_continuous_acceleration_8_bit(DevicePrivate *device_p, Packet *packet) {
	...
	*(void **)(&callback_function) = device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + ACCELEROMETER_V2_CALLBACK_CONTINUOUS_ACCELERATION_8_BIT];
...
void accelerometer_v2_create(AccelerometerV2 *accelerometer_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(accelerometer_v2, uid, ipcon->p, 2, 0, 0);
...
void accelerometer_v2_register_callback(AccelerometerV2 *accelerometer_v2, int16_t callback_id, void *function, void *user_data) {
